					...
					if(tag.soleTag) {
						...
						if(tag.tagName == "text") {
							...
							if(value!="") {
								...
								if(pel && (pel.tagName == "STYLE" || pel.tagName == "OPTION" || pel.tagName == "SPAN")) {
...
Class.define("UI.Body",{ from : ["Component"] , ctor :
	function(){
	...
	Class.define("Router",{
		...
		proto : {
			insert : function(name,load_callback,unload_callback) {
				...
				if(Object.prototype.toString.apply(load_callback) != "[object Function]") throw new Error("load_callback must be function");
...
function Link(target) {
	...
	this.target = target;
	this.schema = target.elementPushPacket(`<span class="Link" id="link" style="cursor:pointer;">${caption}</span>`)
	this.url = url;
	...
	if(url!=null) {
		this.schema.el.link.addEventListener("click",function() {
			window.location = url;
		}); 
	} else {
